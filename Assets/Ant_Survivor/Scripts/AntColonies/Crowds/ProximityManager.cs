...
public class ProximityManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        meshTemplate2 = GameObject.Find("MeshTemplate3");

        //3rd Template with components and materials for building a new object easily
        ...
        for(int i = 0; i < GOList.Length; i++)
        {
            if (GOList[i].name.Contains("nackedSet"))
            {
                ...
                if(npcMeshes.Count < j + 1)
                {
                    ...
                    {
                        if (GOList[j].transform.GetChild(k).GetComponent<SkinnedMeshRenderer>() != null)
                        {
    ...
    /// </summary>
    /// <param  name="gO">GameObject to be baked</param>param>
    /// <param name="j">jth element</param>
    void BakeIt(GameObject gO, int j)
    {
        Vector3 npcLoc = gO.transform.position;

        //get animation and call the SampleAnimation function on the ith element
        ...
        SkinnedMeshRenderer[] listOfSkins = gO.GetComponentsInChildren<SkinnedMeshRenderer>();
        for(int i = 0; i < listOfSkins.Length; i++)
        {
            listOfSkins[i].BakeMesh(npcMeshesGO[j][i].GetComponent<MeshFilter>().mesh);
